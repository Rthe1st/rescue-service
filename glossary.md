...
- **Fire hose** — an object a player character can carry, tracked as an entry in
  ...
  marking its loose end. The "Pick up hose"/"Drop hose" button toggling this sits paired
  with the spray button (see **Spray**, below) as a single action-button cluster
  (`createActionButtons`) - to the right of the D-pad in portrait, or to the right of the
...
- **Spray** — extinguishes fire at range instead of moving. Only available while carrying
  ...
  only visible then) arms aiming mode without ending the turn, and while armed shows
  "Cancel" in place of its usual 💦; the next arrow button press fires in that direction
  instead of moving, searching up to `hoseSprayRange` tiles in a straight line from the
  carrying player, stopping at the first wall, and extinguishing only the *nearest* flame
  tile hit along that line - flames further away on the same line are left burning
  (`sprayHose`). While armed, every tile a spray in some direction could reach is marked
  with a 💧 icon on the board itself (`updateSprayTargetIcons`) - dimmed on a tile that
  isn't on fire (the water would just pass over it) and full opacity on the one tile per
  direction, if any, that would actually be extinguished - so the player can see where
  each arrow press would fire before committing to one. Pressing the spray button again
  while already armed (now reading "Cancel") disarms aiming mode without firing. Firing
  (or losing eligibility to spray, e.g. the burn phase starting) also disarms aiming mode.
  Firing doesn't move the player or change the hose's path; like a move, it ends the
